...
tags: ["query:av-pnc"]
score: 9.0
evidence: 用视觉-语言-动作模型生成自动驾驶连续动作轨迹
tldr: 视觉-语言-动作（VLA）模型在自动驾驶中推理效率较低，且难以泛化到新型车辆配置与驾驶场景。为此，本文提出Reasoning-VLA，采用可学习的动作查询，在空间表征引导下与推理增强的视觉-语言特征交互，并行生成连续动作轨迹。模型整合了八个公开数据集进行训练与评估，实验结果表明其推理高效、泛化能力强，为自动驾驶的规划与控制提供了高效的端到端解决方案。
source: ICML-2026-Accepted
selection_source: conference_retrieval
motivation: 现有VLA模型在自动驾驶中推理开销大，对新型车辆配置与场景泛化不足。
method: 使用可学习动作查询结合空间引导与视觉-语言特征并行生成连续动作轨迹。
result: 在八个公开数据集上验证了高效推理和强泛化能力，优于现有方法。
conclusion: 为自动驾驶提供了一个高效、可泛化的动作生成范式，推进VLA在驾驶决策中的应用。
---
