...
tags: ["query:av-pnc"]
score: 6.0
evidence: 面向离线决策的扩散轨迹规划与动力学建模，可用于自动驾驶规划控制
tldr: 离线决策中的扩散模型常生成与系统动力学不一致的轨迹，可靠性受限。本文提出MPDiffuser，在采样过程中交替进行规划器与动力学扩散模型的更新，逐步修正可行性的同时保持任务意图，并用轻量排序模块挑选最优轨迹。该方法提升了轨迹的动力学合理性、采样效率和适应能力。组合式设计也使动力学模型能够利用多样化的数据，适用于自动驾驶等控制场景。
source: ICML-2026-Accepted
selection_source: conference_retrieval
motivation: 离线决策中的扩散轨迹往往违反系统动力学，导致控制可靠性不足。
method: 提出MPDiffuser，交替扩散规划器和动力学模型，并在采样后排序选择满足任务目标的轨迹。
result: 生成轨迹在动力学可行性和任务对齐上更优，采样效率与泛化能力提升。
conclusion: 该组合式扩散框架为离线预测控制提供可靠且可适应的轨迹生成方法。
---
