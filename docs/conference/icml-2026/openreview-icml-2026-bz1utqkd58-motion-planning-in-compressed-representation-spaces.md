...
tags: ["query:av-pnc"]
score: 9.0
evidence: 面向机器人及自动驾驶车辆的压缩表示空间运动规划，融合深度生成与搜索优化。
tldr: 现有机器人运动规划要么依赖深度模型但可解释性与灵活性不足，要么依赖搜索优化但计算开销大。该文提出在高度压缩的离散标记潜在空间中进行规划的新生成框架，将二者统一。学习到的分层离散表示降低了规划复杂度，同时保留模型先验带来的行为合理性。实验显示该方法在导航与操作任务上既保持规划有效性又提升效率，对自动驾驶规划控制具有参考价值。
source: ICML-2026-Accepted
selection_source: conference_retrieval
motivation: 深度学习方法在运动规划中擅长利用数据先验，但搜索/优化方法仍不可或缺，两者缺乏统一框架。
method: 学习高压缩比自编码器，构建层次化离散标记的潜在空间，并在该表示空间内进行生成式规划。
result: 在机械臂操作与自动驾驶导航等任务上验证了高压缩离散空间规划的有效性与效率。
conclusion: 为学习与规划的统一提供了一种生成式方案，可提升复杂机器人轨迹规划的能力。
---
